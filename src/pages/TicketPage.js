...
import { saveTicket, updateTicket } from "../actions";
...
class TicketPage extends Component {
  ...
  save = (ticket) => {
    if (!ticket.uuid) {
      this.props.saveTicket(
        ...
        ticket,
        formatMessageWithValues(this.props.intl, "ticket", "saveTicket.mutationLabel", {
  ...
  render() {
    const { classes, modulesManager, history, rights, ticket_uuid } = this.props;
    if (!rights.includes(RIGHT_TICKET_EDIT)) return null;
    return (
      <div className={classes.page}>
        <TicketForm
          ticket_uuid={ticket_uuid !== "_NEW_" ? ticket_uuid : null}
...
const mapDispatchToProps = (dispatch) => {
  return bindActionCreators({ saveTicket, updateTicket }, dispatch);
};

export default withHistory(
  withModulesManager(
    connect(mapStateToProps, mapDispatchToProps)(injectIntl(withTheme(withStyles(styles)(TicketPage)))),
  ),
);
